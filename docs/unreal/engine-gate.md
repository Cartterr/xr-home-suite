...
- Slate launcher platform icons: restored under `Engine\Content\Editor\Slate\Launcher` for Android, iOS, Linux, Mac, and tvOS to avoid missing-resource launch warnings from the Win64-focused source install.
...
`python -m xrhs unreal-doctor` reports source access, local engine markers, the editor binary, `ShaderCompileWorker`, Slate launcher icon readiness, and the Meta plugin staging state.
